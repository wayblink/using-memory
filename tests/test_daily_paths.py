...
class DailyPathTests(unittest.TestCase):
    def test_docs_name_the_canonical_flat_daily_path(self):
        for rel in [
            ...
            "examples/memory-repo/local/WORKSPACE.md",
        ]:
    ...
    def test_docs_do_not_publish_legacy_markdown_daily_path(self):
        legacy_path = "daily/YYYY-MM-DD.md"
        for rel in [
            "SKILL.md",
            "README.md",
            "references/startup-and-write-rules.md",
    ...
    def test_schema_does_not_publish_removed_commands_or_flags(self):
        text = (ROOT / "examples/memory-repo/SCHEMA.md").read_text(encoding="utf-8")
